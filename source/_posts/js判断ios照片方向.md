...
```html
<!-- label绑定file被点击开始拍照 -->
<label id="filebtn" for="file" style="position:absolute;left:0;top:0;background:rgba(255,0,0,.01);width:100%;height:100%;z-index:9999999;display:none;"></label>
<!-- file调用摄像机拍照 -->
<input type="file" id="file" accept="image/jpeg" capture="camera" style="display:none;" />
<script src="js/exif.js"></script>
```
```js
let [w,h] = [window.innerWidth,window.innerHeight]     //获取浏览器的宽高
let image = new Image() //新建一个图片对象 存储导入的图片
let orientation = 0;//存储照片的方向值 3 6 8
//inputfile改变事件 获取inputfile中的图片
$("#file").on('change',function(event){
    ...
    EXIF.getData(file, function () {
        ...
        //console.log(allTags,orientation)
        //alert(orientation)  //照片方向
    })
    //新建一个图片
...
function SetImg(){
    ...
    switch (orientation) {
        case 6 :
            //ios可能是6 但是不需要旋转 所以此处用宽度大于高度的话就需要旋转 否则不需要旋转
            if(w >= h){
                //需要顺时针（向左）90度旋转
